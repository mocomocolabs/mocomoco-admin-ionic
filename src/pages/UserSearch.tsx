              ...
              <div className='result-list-wrap'>
                ...
                {$user.getSearchResultList && $user.getSearchResultList.count === 0 ? (
                  ...
                  <div className='flex-center mt-3 mb-3'>검색 결과는 (( {$user.getSearchResultList.count} )) 건 입니다. 
                    (<span>승인대기</span>
                    <span className='result_cnt-pending-red'>{$user.getSearchResultList?.userList?.filter(a => a.status === SIGN_UP_STATUS.PENDING).length}</span>건 포함)
                  </div>
                )
              }
                  ...
                  return (
                      ...
                      <IonItem lines="none"> 
                        ...
                        <IonLabel>
                          ...
                          {item.status !== SIGN_UP_STATUS.APPROVAL ? (<h5 className='pending-red-in-list ml5'>승인대기</h5>) : <></> }
