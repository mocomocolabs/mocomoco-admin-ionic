...
import { arrowDown, chevronDownOutline, chevronUpOutline, closeCircle, refreshOutline } from 'ionicons/icons'
...
import { ICommunityUsers } from '../stores/auth-store'
...
export const Home: React.FC = () => {
  ...
  const [isShowDtlArr, setIsShowDtlArr] = useState<number[] | undefined>()
      ...
      <IonContent>
        <div className='px-container'>
                ...
                {/* <IonList> */}
                  { usersListToApprove && usersListToApprove.map((item, index) => (
                    <>
                      <IonItem> 
                        <IonIcon
                          icon={isShowDtlArr?.includes(item.id) ? chevronDownOutline : chevronUpOutline}
                          className='absolute right-0 bottom-0 mt10'
                          onClick={() => {
                            isShowDtlArr?.includes(item.id) ? isShowDtlArr?.splice(isShowDtlArr.indexOf(item.id), 1) : setIsShowDtlArr([item.id])
                          }}
                        ></IonIcon>
                        {/* <IonAvatar slot="start">
                          <IonImg src={item.profileUrl} />
                        </IonAvatar> */}
                        {/* <IonCheckbox
                          className='mr5'
                        ...
                        <IonLabel>
                          <h2>{item.name}</h2>
                          <h3>{ymdhm(item.createdAt)}</h3>
                          { isShowDtlArr?.includes(item.id) ? <p>{item.introduce}</p> : null }
                        </IonLabel>
                      </IonItem>
                    </>
                  ))}
          ...
          {/* <div className='month-event-wrap'>
            ...
            <IonCard>
              <IonItem>
                <IonLabel>ionItem in a card, icon left, button right</IonLabel>
